    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {isLoading
            ? Array.from({ length: 6 }).map((_, i) => <ProcessCardSkeleton key={i} />)
            : processFlows.map((flow) => (
                <ProcessCard key={flow.id} process={flow} onSelect={setSelectedProcess} />
              ))}
        </div>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => { setSelectedProcess(null); setCurrentStep(1) }}
        ...
        <div className="flex-1">
          <h1 className="text-xl font-bold text-foreground">{selectedFlow?.title}</h1>
      ...
      <div className="rounded-xl border border-border bg-card p-4 lg:p-6">
        <Stepper steps={stepLabels} currentStep={currentStep} onStepClick={setCurrentStep} />
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5 lg:p-6">
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <div>
            <Label htmlFor="amount" className="text-foreground">Estimated Amount (₦)</Label>
            <Input
              id="amount"
              type="number"
              placeholder="0.00"
          ...
          <div className="sm:col-span-2">
            <Label htmlFor="justification" className="text-foreground">Business Justification</Label>
            <Textarea
              id="justification"
        ...
        {currentUser && (
          <div className="mt-4 flex items-center gap-2 rounded-lg bg-gray-900/50 border border-gray-800 px-4 py-2.5">
            ...
            <span className="text-xs text-gray-400">
              Submitting as <span className="text-white font-medium">{currentUser.displayName}</span>
